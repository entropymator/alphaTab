...
function isElementStyleHelperUsing(
    ...
    // Accept either a direct `ElementStyleHelper.X(...)` call or a
    // `cond ? ElementStyleHelper.X(...) : undefined`-style conditional
    // chain whose leaves are each an ElementStyleHelper call or
    // undefined/void 0. The output rewrite is identical for both shapes
    // because the optional-chain dispose handles `undefined` already.
