...
export class BarRendererBase {
    ...
    public afterReverted() {
        ...
        // DR-1 (v2): `_voiceWalkDone` ALSO survives `afterReverted`.
        // `afterReverted` fires on every resize cycle for every renderer
        // via the `_resizeAndRenderScore` loop, so invalidating here would
        // defeat the walk-skip optimisation entirely. The walk's broker
        // outputs are bar-local invariant per the §2.2 catalogue.
    }
