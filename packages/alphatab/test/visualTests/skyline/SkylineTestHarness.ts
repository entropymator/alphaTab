...
export interface BarSkylineSnapshot {
    barIndex: number;
    renderer: 'score' | 'tab' | 'slash' | 'numbered' | 'other';
...
function classifyRenderer(staffId: string): BarSkylineSnapshot['renderer'] {
    if (staffId.indexOf('score') >= 0) return 'score';
    if (staffId.indexOf('tab') >= 0) return 'tab';
    if (staffId.indexOf('slash') >= 0) return 'slash';
    if (staffId.indexOf('numbered') >= 0) return 'numbered';
    return 'other';
}
            ...
            for (const staff of group.staves) {
                ...
                for (const renderer of staff.barRenderers) {
                    ...
                    bars.push({
                        ...
                        renderer: classifyRenderer(staffId),
...
export async function renderSkylineOnce(tex: string, width: number = 1300): Promise<StaffSkylineSnapshot[]> {
...
export function findScoreStaff(snapshots: StaffSkylineSnapshot[], trackIndex: number = 0): StaffSkylineSnapshot {
...
export function findTabStaff(snapshots: StaffSkylineSnapshot[], trackIndex: number = 0): StaffSkylineSnapshot {
...
export function findSlashStaff(snapshots: StaffSkylineSnapshot[], trackIndex: number = 0): StaffSkylineSnapshot {
...
export function findNumberedStaff(snapshots: StaffSkylineSnapshot[], trackIndex: number = 0): StaffSkylineSnapshot {
 ...
 */
export function maxUpHeightInRange(bar: BarSkylineSnapshot, xStart: number, xEnd: number): number {
    let max = 0;
 ...
 * within the given bar snapshot. Returns 0 if no overlapping segment.
 */
export function maxDownHeightInRange(bar: BarSkylineSnapshot, xStart: number, xEnd: number): number {
    let max = 0;
