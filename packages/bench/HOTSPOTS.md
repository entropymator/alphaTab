...
| EW-8 | `2251590d` | canon-resize | -0.69 ms (-1.3 % ★); fade-to-black-resize -2.10 ms (-9.8 % `·`) | 2026-06-14 | Short-circuit `SvgCanvas._escapeText` with a single character-class `test()` when the input has no `& < > " '`. Bench corpus is mostly numeric text (bar numbers, time signatures, fret numbers, tuplet labels) → 5 sequential global-regex passes collapse to 1 `test()` on the common path. A/B at n=64 (`probe-EW-8`): canon-resize `★` CI [-1.00, -0.22] z=2.50. Smaller absolute win than the HOTSPOTS estimate predicted, but cleanly above the σ floor on the target. vitest 1599/1599. |
